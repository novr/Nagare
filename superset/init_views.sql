...
GROUP BY r.repository_name, pr.status, DATE((pr.started_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo'));
...
JOIN repositories r ON pr.repository_id = r.id
WHERE pr.started_at IS NOT NULL
GROUP BY r.repository_name, DATE((pr.started_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo'));
...
SELECT
    ...
    (pr.completed_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo') as completed_at,
...
SELECT
    ...
    j.job_name,
    j.status,
    DATE((j.started_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo')) as run_date,
    COUNT(*) as job_count,
...
GROUP BY r.repository_name, j.job_name, j.status, DATE((j.started_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo'));

-- 5. 時間帯別のパイプライン実行数（JST）
CREATE OR REPLACE VIEW v_pipeline_runs_by_hour AS
SELECT
    r.repository_name,
    EXTRACT(HOUR FROM (pr.started_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo')) as hour_of_day,
    COUNT(*) as run_count,
    AVG(pr.duration_ms) / 1000.0 as avg_duration_sec
FROM pipeline_runs pr
JOIN repositories r ON pr.repository_id = r.id
WHERE pr.started_at IS NOT NULL
GROUP BY r.repository_name, EXTRACT(HOUR FROM (pr.started_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo'));
...
SELECT
    ...
    MAX((pr.started_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Tokyo')) as last_run_time,
    SUM(CASE WHEN pr.status = 'SUCCESS' THEN 1 ELSE 0 END) as success_count,
    SUM(CASE WHEN pr.status = 'FAILURE' THEN 1 ELSE 0 END) as failure_count,
