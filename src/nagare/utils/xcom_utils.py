...
def split_data_for_xcom(
    data: list[Any], key_prefix: str, max_chunk_size: int = XCOM_SIZE_WARNING_THRESHOLD
) -> list[tuple[str, list[Any]]]:
    """大きなリストデータをXCom用に分割する

    Args:
        data: 分割するリストデータ
        key_prefix: XComキーのプレフィックス
        max_chunk_size: チャンクの最大サイズ（バイト）

    Returns:
        (key, chunk_data)のタプルのリスト
    """
    if not data:
        return [(key_prefix, [])]
    ...
    for item in data:
        item_size = get_data_size(item)

        # 現在のチャンクに追加すると制限を超える場合、新しいチャンクを作成
        if current_chunk and current_size + item_size > max_chunk_size:
            chunk_key = f"{key_prefix}_chunk_{chunk_index}"
            chunks.append((chunk_key, current_chunk))
            current_chunk = []
            current_size = 0
            chunk_index += 1
    ...
    if current_chunk:
        if chunk_index == 0:
            # チャンクが1つだけの場合はsuffixなし
            chunks.append((key_prefix, current_chunk))
        else:
            chunk_key = f"{key_prefix}_chunk_{chunk_index}"
            chunks.append((chunk_key, current_chunk))

    logger.info(f"Split data into {len(chunks)} chunk(s) for XCom")
    return chunks
